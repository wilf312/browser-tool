...
待機画面の左下に「10:15 に自動で参加します」とカウントダウンが表示され、**キャンセル**ボタンでその場で取りやめられます。もう少しだけ待ってほしいときは **+1 分** ボタンで入室時刻を 1 分ずつ後ろにずらせます（押した回数だけ延びます）。入室・キャンセル後の表示は数秒で消えますが、待たずにパネルのどこかをクリックすればすぐ消せます（カウントダウン中のクリックでは消えません）。
