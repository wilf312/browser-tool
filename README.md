...
入室をリクエストしたあと、Meet から「まだ参加できないようです。このまま待機を続けますか？」（**退出** / **待機**）と聞かれることがあります。放っておくと待機列から外されてしまうので、このダイアログが出たら自動で **待機** を押します。
...
- 「このまま待機を続けますか？」のダイアログは、ダイアログ（`role="dialog"` / `role="alertdialog"`）の中にある `待機` / `Keep waiting` ボタンだけを対象にします。隣の `退出` を誤って押さないよう、ラベルは完全一致で判定します
- ダイアログはカウントダウンが消えたあと（入室をリクエストしてから数分後）に出るため、待機画面を開いている間はカウントダウンとは別に監視し続けます
...
[ページ自動リロード]
  ...
  リロード間隔 [5 分 ▾]  続ける時間 [1 時間 ▾]  [開始]
                            ↓
  5 分ごとにリロードしています（終了まで 58:42 ／ 次まで 04:18）  [停止]
```

- **リロード間隔**: 5 分 / 10 分 / 30 分。既定は 5 分です
- **続ける時間**: 1 時間から 8 時間まで 1 時間きざみ。既定は 1 時間です。この時間がたてば自分で
  止まります。止め忘れて丸一日リロードし続ける、ということにはなりません
...
- リロードは [`chrome.alarms`](https://developer.chrome.com/docs/extensions/reference/api/alarms) が起こします。service worker は次のリロードまでの間に
  停止されますが、アラームが来れば起き直すので、間隔が何分空いても関係ありません（Chrome の
  アラームは 30 秒より短い間隔では動きませんが、選べる最短が 5 分なので届きません）
...
```json
{
  ...
  "features": {
    ...
    "reloadTimer": { "intervalSeconds": 300, "durationMinutes": 60 }
  }
}
```
