...
|          | カテゴリ                                  | 理由                                                                                                                                                                                                                                                           |
| -------- | ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **推奨** | ワークフローと計画（Workflow & Planning） | どの機能も「仕事中のブラウザ作業から手数を減らす」もので、チケット管理（Jira）・会議（Meet）・コードレビュー（GitHub）という業務の動線そのものに乗っています。同カテゴリには会議・タスク・社内ツール系の拡張が集まっていて、想定ユーザーの探し方に一番近いです |
...
道具は今のところ 4 つ。どれも既定では何もせず、あなたが設定した内容・操作にだけ反応します。
...
Pull Request のページで、CI のチェックがまだ終わっていない間だけ、「Merge pull request」ボタンの
横に専用のボタンが現れます。押すと ON になり、チェックがすべて green になった時点でマージまで
自動で完了させます。

・既定では何もしません。ボタンを押して ON にしたときだけ監視が始まります
・チェックが 1 つでも失敗すると、マージはせずに監視を止めます
...
・github.com（Pull Request のページのみ） — CI のチェック状況を読み取り、ユーザーが自動マージを
  ON にしたときに「Merge pull request」ボタンと、その後の確認ボタンをクリックするためのものです
...
Chrome Web Store は「拡張機能の目的は 1 つに絞られていること」を求めます。Jira・Meet・GitHub という
...
参加ボタンを押すこと、更新を待つページを一定時間リロードし続けること、Pull Request の
CI がすべて green になったらマージボタンを押すことは、いずれもユーザーが手で行っている
同じ操作の置き換えです。
```

3 つ以上の外部サービスを 1 つの拡張機能で扱っている点は、この文面だけでは押し切れない可能性が
あります。差し戻された場合の逃げ道は、Meet 自動入室・GitHub 自動マージをそれぞれ別アイテムとして
分けて出すことです。コードは `src/lib/meet-*` / `src/lib/github-*` と `src/content/` にまとまって
いて、manifest から該当する content script の match と host_permissions を落とせば、他の機能だけの
ビルドになります。

### 権限の正当性

権限ごとに欄が分かれています。`manifest.json` が要求しているのは次の 7 つで、それ以外の欄は出ません。
...
**コンテンツ スクリプト（`https://github.com/*/*/pull/*`）の理由**

```
GitHub の Pull Request ページに、CI のチェックがすべて green になったら自動でマージするボタンを表示するために使用します。ユーザーがこのボタンを押して自動マージを ON にした場合だけ、ページに表示されているチェックの要約（成功・失敗・実行中）を読み取り、すべて成功した時点でユーザーに代わって「Merge pull request」ボタンとその後の確認ボタンをクリックします。ボタンを押さない限り何もしません。対象は Pull Request 自身のページだけで、読み取った内容を外部へ送信することはありません。
```
...
4. GitHub の Pull Request ページで、「Merge pull request」の横に自動マージのボタンが出ている状態
5. 設定のインポート / エクスポート（機能ごとのチェックボックスが見えている状態）
